...
const orbitalPeriod = (position, velocity) => {
  ...
  const positionVectorMagnitude = Math.sqrt(
    Math.pow(x, 2) + Math.pow(y, 2) + Math.pow(z, 2)
  );
  const velocityVectorMagnitude = Math.sqrt(
    Math.pow(vx, 2) + Math.pow(vy, 2) + Math.pow(vz, 2)
  );

  const semiMajorAxis =
    positionVectorMagnitude /
    (1 -
      (Math.pow(velocityVectorMagnitude, 2) * EARTH_RADIUS_KM) /
        GRATIVATIONAL_EARTH_PARAMETER);

  const orbitalPeriod =
    (2 * Math.PI * Math.pow(semiMajorAxis * 1000, 1.5)) /
    Math.sqrt(GRATIVATIONAL_EARTH_PARAMETER);
...
const getPointsCountForOrbit = (orbitDuration, interpolationDegree) => {
  ...
  const increasedDistance =
    Math.round(orbitDuration / increaseStep) * increaseTime;
...
const isPropagatedObjectValid = (object) => {
  if (!object || !object.position || !object.velocity) return false;
  const positionValid = !Object.values(object.position).some((val) =>
    isNaN(val)
  );
  const velocityValid = !Object.values(object.velocity).some((val) =>
    isNaN(val)
  );
...
const createPropagatedArray = (
  ...
  satrecs.forEach(({ record, name }) => {
    ...
    if (isPropagatedObjectValid(propagated)) {
      let periodMinutes = orbitalPeriod(
        propagated.position,
        propagated.velocity
      );
      ...
      if (!isNaN(periodMinutes)) {
        ...
        const pointsCount = getPointsCountForOrbit(
          orbitDuration,
          interpolationDegree
        );
        const times = [];
        ...
        const averagePeriod =
          periodsSum > 0 ? periodsSum / (pointsCount - 1) : 0; // minutes
